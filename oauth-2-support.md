...
NanoIoT allows you to provide Single Sign On functionality for your customers and automatically create tenants, customers or subcustomers using external user management platforms, that supports OAuth 2.0 protocol.  
...
NanoIoT supports the Authorization Code grant type to exchange an authorization code for an access token.   
Once user returns to the NanoIoT client via redirect URL, the platform will get the authorization code from the URL and use it to request an access token from the external user management platform.
Using the [basic mapper]) or [custom mapper]external user info object will be converted from external platform into NanoIoT internal OAuth 2.0 user and then the regular NanoIoT authorization flow will happen.
...
Please add to Authorized redirect URIs section default NanoIoT redirect URI that we are going to use in this sample:
...
#### Configuration of NanoIoT

You will need to modify OAuth 2.0 parameters in [NanoIoT.yml]
...
security:
  ...
  oauth2:
    ...
    # For details please refer to https://NanoIoT.io/docs/user-guide/oauth-2-support/
    ...
    clients:
      default:
        ...
        mapperConfig:
          ...
          # Allows user to setup NanoIoT internal password and login over default Login window
...
So that the resulted NanoIoT.yml oauth2 configurations for Google will look similar to the provided below:

If we navigate to the Login screen, we will see additional Login option with Google:

Once we click it and select on of our Google Account, we are going to be logged into NanoIoT with our Google's email as a Tenant Administrator email:
...
Now it is time to update the [NanoIoT.yml] with the additional SSO provider. 
...
So that the resulted NanoIoT.yml oauth2 configurations for OAuth0 will look similar to the provided below:


If we navigate to Login screen, we will see two possible Login with options - **Google** and **Auth0**:


Once we click it and select our *Auth0* Account, we are going to be logged into NanoIoT with our email's as Customer User:
...
We have completed our sample and now your users not required to create accounts inside NanoIoT - they can use already exist SSO providers for this.
...
Main functionality of the mapper is to map key-value attributes from the external user info object into expected structure of the NanoIoT OAuth 2.0 User:
...
A basic mapper is able to merge external OAuth 2.0 user info object into NanoIoT OAuth 2.0 user with a predefined set of rules. 
Configuration of this mapper done over the [NanoIoT.yml]:
...
- **allowUserCreation**
  If this option set to **true**, then if user account does not exist in the NanoIoT yet, it will be created.
  If this option set to **false**, user will get access denied error if will try to login with an external OAuth 2.0 provider, but there is no user exists with those credentials in the NanoIoT.   
 
- **emailAttributeKey**
  This is the key of the attributes from the external OAuth 2.0 user info that is going to be used as NanoIoT user email property.
  
- **firstNameAttributeKey**
  This is the key of the attributes from the external OAuth 2.0 user info that is going to be used as NanoIoT user first name property.
    
- **lastNameAttributeKey**
  This is the key of the attributes from the external OAuth 2.0 user info that is going to be used as NanoIoT user surname property.
...
A custom mapper designed as a separate microservice which is running nearby the NanoIoT core microservice.
NanoIoT forwards all mapping requests to this microservice and expects as a response NanoIoT OAuth 2.0 user object:
...
Configuration of this mapper done over [NanoIoT.yml]:
...
If NanoIoT is running under loadbalancer like HAProxy please configure properly balance algorithm to make sure that the correct session is available on the NanoIoT instance: 
```bash
