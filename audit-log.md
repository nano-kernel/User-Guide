...
NanoIoT provides the ability to track user actions in order to keep audit log. 
...
It is possible to fetch audit logs via [REST API]. 
...
System administrator is able to configure audit log levels using [NanoIoT.yml](/docs/user-guide/install/config/). You can find sample configuration below:
...
For all other entities, NanoIoT will log only write level operations.
