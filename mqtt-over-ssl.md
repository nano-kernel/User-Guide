...
title: MQTT over SSL
description: Launching NanoIoT with secure MQTT protocol to connect your IoT devices and projects.
...
NanoIoT provides the ability to run MQTT server over SSL. Both one-way and two-way SSL are supported.
To enable SSL, you will need to obtain a valid or generate a self-signed SSL certificate and add it to the keystore.
Once added, you will need to specify the keystore information in **NanoIoT.yml** file.
See the instructions on how to generate SSL certificate and use it in your NanoIoT installation below.
...
This script will run keytool using the configuration specified. It will generate the following output files:
 ...
 - **SERVER_FILE_PREFIX.jks** - Java keystore file. This is the file which will be used by NanoIoT MQTT Service
...
```bash
sudo chmod 400 /etc/NanoIoT/conf/mqttserver.jks
sudo chown NanoIoT:NanoIoT /etc/NanoIoT/conf/mqttserver.jks
```

### Server configuration

Locate your **NanoIoT.conf** file and set the MQTT_SSL_ENABLED value equals  true.

You can add the next row for to the **NanoIoT.conf**, so that the MQTT over SSL will be enabled.  
```bash 
...
...
The MQTT bind port can be changed with the next row within the **NanoIoT.conf** being added:
...
**NOTE:** NanoIoT supports **.p12** keystores as well. if this is the case, set **key_store_type** value to **'PKCS12'**

After these values are set, launch or restart your NanoIoT server.

### Example of configuration 
The next combination of the **keygen.properties** example was used to generate a proper .jks and .pem in a case of the NanoIoT uses the next default **NanoIoT.conf** with the enchantments being specified below.   
This example is based on the default NanoIoT installation of the **2.5 version**. 
